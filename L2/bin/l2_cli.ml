...
let get_json testcase runtime solution config argv : Json.t =
...
let synthesize ?spec_dir ?max_cost engine deduction cost_model library testcase =
  ...
  match testcase.T.case with T.Examples (exs, bg) -> (
    ...
    | `V2 -> (
        ...
        let deduce =
          List.fold_left deduction ~init:Deduction.no_op ~f:(fun d -> function
        ...
        | Ok (Some s) ->
            let hypo_str =
              Pp.to_string ~width:70 (Skeleton.to_pp (Hypothesis.skeleton s))
            in
    ...
    +> flag "-dd" ~aliases:["--deduction"]
         (optional_with_default [`Higher_order]
            (nonempty_symbol_list
               ...
               ; ("none", `None) ]))
         ~doc:
           " deduction routines to use during synthesis (only applies when using \
            v2 engine)"
...
let commands =
  Command.group
    ~summary:"A suite of tools for synthesizing and running L2 programs."
